...
int main(int argc, char** argv) {
  ...
  const std::string base_path  = "./data/sift100m/base.100M.f32bin";
  const std::string data_root  = argc > 1 ? argv[1] :
    "./indexes/sift100m";
  const std::string shards_dir = data_root + "/shards";
  const std::string tag_vocab_path = std::getenv("BCI_SHARD_TAG_VOCAB") ?
    std::string(std::getenv("BCI_SHARD_TAG_VOCAB")) :
    data_root + "/sweet_spot_tags.bin";
