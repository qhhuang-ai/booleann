...
int main(int argc, char** argv) {
  ...
  const std::string DRT  = "./data_laion";
  ...
  csr_filters qm(DATA + "query.metadata.laion1m.richer.spmat");
  csr_filters bm(DATA + "base.metadata.laion1m.richer.spmat");
  ...
  // LAION GT: single file gt.10K.richer.bin with all 10000 queries indexed 0..N
  std::string gt_path = DATA + "gt.10K.richer.bin";
  ...
  // For 1326 sweet-spot tags = 1.66GB total memory. O(1) bit lookup per check.
  ...
  for (auto& tc : clusters) {  // iterate same tag set as clusters
    int32_t tag = tc.first;
    auto& bv = bitvecs[tag];
